...
export const listTransactionAccounts = requireAuth(async (event) => {
  const accounts: Array<Selectable<ChartOfAccountsTable>> = await event.context.db
  ...
  return { data: accounts.map((account) => serializeAccount(account as any)) };
});

export const currentChartOfAccounts = requireAuth(async (event) => {
