...
export const createInviteHandler = defineEventHandler({
	...
	handler: async (event) => {
		const payload = await readValidatedBody(event, createInviteSchema.safeParse)
...
export const completeInviteHandler = eventHandler(async (event) => {
	const params = getRouterParams(event)
	const payload = await readValidatedBody(event, completeInviteSchema.safeParse)
...
export const updateInviteHandler = defineEventHandler({
	...
	handler: async (event) => {
		const params = getRouterParams(event)
		const payload = await readValidatedBody(event, updateInviteSchema.safeParse)
