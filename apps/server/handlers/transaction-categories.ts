import { getQuery, getRequestURL, getRouterParams, setResponseStatus } from "h3";
...
export const listTransactionCategories = requireAuth(async (event) => {
  ...
  const categories: Array<Selectable<TransactionCategoryTable>> = await categoriesQuery
    .orderBy("name")
    .limit(limit)
    .offset(offset)
    .execute();
  ...
  const accounts: Array<Selectable<ChartOfAccountsTable>> = accountIds.length
...
export const getTransactionCategoryDefaults = requireAuth(async (event) => {
